<?php
/*
Plugin Name: Redmatrix-wp : Crosspost to Redmatrix
Plugin URI: http://blog.duthied.com/2011/09/12/friendika-cross-poster-wordpress-plugin/
Description: This plugin allows you to cross post to your Redmatrix account. Extended by Mike Macgirvin from a Friendica cross-posting tool written by Devlon Duthied
Version: 1.3
Author: Mike Macgirvin, Devlon Duthied
Author URI: https://zothub.com/channel/mike
*/
...
function redmatrix_wp_deactivate() {
	delete_option('redmatrix_wp_hub_location');
	delete_option('redmatrix_wp_acct_name');
	delete_option('redmatrix_wp_user_name');
	delete_option('redmatrix_wp_password');
}

function redmatrix_wp_get_hub_location() {
	return get_option('redmatrix_wp_hub_location');
}
...
function redmatrix_wp_post($post_id) {
    ...
    if (isset($_POST['redmatrix_xpost'])) {
		update_post_meta($post_id, 'redmatrix_xpost', '1');
	} 

	// if meta has been set
	if (get_post_meta($post_id, "redmatrix_xpost", true) === '1') {

		$user_name = redmatrix_wp_get_acct_name();
		$password = redmatrix_wp_get_password();
		$hub_location = redmatrix_wp_get_hub_location();
		$channel = redmatrix_wp_get_channel_name();
		$backlink = get_option('redmatrix_wp_backlink');
		
		if ((isset($user_name)) && (isset($password)) && (isset($hub_location))) {
			...
			$bbcode = redmatrix_wp_html2bbcode($message);

			if($backlink)
				$bbcode .= "\n\n" . _('Source:') . ' ' . '[url]' . get_permalink($post_id) . '[/url]';
			
			$url = $hub_location . '/api/statuses/update';
			
			$headers = array('Authorization' => 'Basic '.base64_encode("$user_name:$password"));
			$body = array(
				'title'     => redmatrix_wp_html2bbcode($post->post_title),
				'status'    => $bbcode,
				'source'    => 'WordPress', 
				'namespace' => 'wordpress',
...
function redmatrix_wp_delete_post($post_id) {
	...
	// if meta has been set
	if ((get_post_meta($post_id, "redmatrix_xpost", true) == '1') || (get_post_meta($post_id, "post_from_red", true) == '1')) {
		...
		if ((isset($user_name)) && (isset($password)) && (isset($hub_location))) {
			...
			$message_id = site_url() . '/' . $post_id;
			$url = $hub_location . '/api/statuses/destroy';
...
function redmatrix_wp_displayAdminContent() {
	...
	$hub_url = redmatrix_wp_get_hub_location();
	$password = redmatrix_wp_get_password();
	...
	// debug...
	// echo "hub location: $hub_url</br>";
	// echo "password: $password</br>";
	// echo "user_acct: $user_acct</br>";
	
	echo <<<EOF
	<div class='wrap'>
		<h2>CrossPost to Redmatrix</h2>
		<p>This plugin allows you to cross post to your Redmatrix channel.</p>
	</div>
	
	<div class="wrap">
		<h2>Configuration</h2>
		<form method="post" action="{$_SERVER["REQUEST_URI"]}">
			Enter the login details of your Redmatrix account<br /><br />
			...
			Redmatrix hub URL: <input type="text" name="redmatrix_wp_url" value="{$hub_url}"/><br />
			Optional channel nickname: <input type="text" name="redmatrix_wp_channel" value="{$channel}"/><br />
			Add permalink to exported posts? <input type="checkbox" name="redmatrix_wp_backlink" value="1" {$backlink_checked} /><br />
...
function redmatrix_wp_display_admin_page() {
	...
	if ((isset($_REQUEST["redmatrix_wp_acct_name"])) && (isset($_REQUEST["redmatrix_wp_password"]))) {
		...
		update_option('redmatrix_wp_hub_location', $red_url);
...
function red_comment($args) {
	...
	if($edit) {
		...
		$comment_ID = $comment['comment_ID'];
       	...
       	$comment_ID = redmatrix_wp_new_comment( $comment );
		if($comment_ID) {
...
function redmatrix_wp_wp_comment($comment_ID,$comment_status) {
	...
	if ((get_post_meta($c['comment_post_ID'], "redmatrix_xpost", true) == '1') 
		...
		if(intval($c['user_id']) && $c['user_id'] == $p['post_author']) {
			...
			if ((isset($user_name)) && (isset($password)) && (isset($hub_location))) {
				$headers = array('Authorization' => 'Basic ' . base64_encode("$user_name:$password"));
				$body = $arr;
				$url = $hub_location . '/api/red/xchan';
				$request = new WP_Http;
				$result = $request->request($url , array( 'method' => 'POST', 'body' => $body, 'headers' => $headers));
				if($result['body']) {
...
function redmatrix_wp_html2bbcode($text) {
	$htmltags = array(
		'/\<b\>(.*?)\<\/b\>/is',
		'/\<i\>(.*?)\<\/i\>/is',
		'/\<u\>(.*?)\<\/u\>/is',
...
function redmatrix_wp_new_comment( $commentdata ) {
